...
def mine_sweeper(bombs, num_rows, num_cols):
    ...
    for bomb in boms:
        row_i, col_i = bombs[0], bombs[1]
        field[row_i][col_i] = -1

        for cell_row in range(row_i - 1, row_i + 1):
            for cell_col in range(col_i - 1, col_i + 1):
                if(0 <= cell_row < num_rows and 0 <= cell_col < num_cols
                        and field[cell_row][cell_col] != -1):
                    field[cell_row][cell_col] += 1
